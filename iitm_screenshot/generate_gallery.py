...
category_counts = {"Active": 0, "Restricted": 0, "Warning": 0, "Error": 0, "Down": 0}
category_urls = {"Active": [], "Restricted": [], "Warning": [], "Error": [], "Down": []}

def get_style(category):
    if category == "Active":
        return "🟢", "green", "badge-active", "border-active"
    elif category == "Restricted":
        return "🟡", "purple", "badge-restricted", "border-restricted"
...
details.stat-box {{ background: white; padding: 25px 20px; border-radius: 10px; box-shadow: 0 4px 6px rgba(0,0,0,0.05); flex: 1; min-width: 200px; border-top: 5px solid #ccc; transition: transform 0.2s, box-shadow 0.2s; }}
...
.border-restricted {{ border-top-color: #6f42c1; }}
...
.card h4 {{ margin: 0 0 10px 0; font-size: 17px; word-break: break-all; }}
.card h4 a {{ color: #0056b3; text-decoration: none; }}
.card p.title-text {{ margin: 0 0 15px 0; font-size: 14px; color: #555; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }}
...
.badge-score {{ background: #e3f2fd; color: #0d47a1; border-color: #bbdefb; }}

.badge-active {{ background: #d4edda; color: #155724; border-color: #c3e6cb; }}
.badge-restricted {{ background: #e0cffc; color: #432874; border-color: #d0bdfa; }}
...
.score-pill {{ padding: 6px 12px; border-radius: 20px; font-weight: 600; font-size: 13px; background: #e3f2fd; color: #0d47a1; }}
</style>
<script>
function toggleView() {{
...
for item in health_data:
    ...
    <div class="card filterable-item" data-ports="{data_ports_attr}">
        ...
        <p class="title-text" title="{page_title}">📄 {page_title}</p>
        <div class="badges">
            <span class="badge {badge_class}">{icon} {category}</span>
            <span class="badge badge-score">Score: {confidence}</span>
            <span class="badge">HTTP: {status_code_str}</span>
            <span class="badge">{response_time}ms</span>
            <span class="badge">Ports: {ports_str}</span>
            <span class="badge" title="Retries used">Retries: {retries}</span>
        </div>
    '''
    if screenshot and os.path.exists(screenshot):
        file_uri = f"raw_screenshots/{os.path.basename(screenshot)}"
        grid_cards += f'''
        <a href="{file_uri}" target="_blank">
            <img src="{file_uri}" alt="Screenshot">
        </a>
        '''
    else:
        grid_cards += '''
    ...
    list_rows += f'''
        <tr class="list-row filterable-item" data-ports="{data_ports_attr}">
            <td>
                ...
                <br><span style="font-size:12px;color:#666;">{page_title}</span>
            </td>
            <td><span class="status-pill {badge_class}">{icon} {category}</span></td>
            <td><strong>{status_code_str}</strong></td>
            <td><span class="score-pill">{confidence} / 100</span></td>
...
<div class="stats-container">
    ...
    {build_dropdown("Restricted", category_counts["Restricted"], "🟡", "border-restricted", category_urls["Restricted"])}
